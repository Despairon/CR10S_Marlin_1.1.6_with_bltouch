...
uint16_t max_display_update_time = 0;
  ...
  #define _MENU_ITEM_PART_2(TYPE, LABEL, ...) \
      ...
      if (lcdDrawUpdate) \
        lcd_implementation_drawmenu_ ## TYPE(encoderLine == _thisItemNr, _lcdLineNr, LABEL, ## __VA_ARGS__); \
    } \
    ++_thisItemNr
  ...
  #define MENU_ITEM_EDIT(type, label, ...) MENU_ITEM(setting_edit_ ## type, label, label, ## __VA_ARGS__)
  #define MENU_ITEM_EDIT_CALLBACK(type, label, ...) MENU_ITEM(setting_edit_callback_ ## type, label, label, ## __VA_ARGS__)
  #if ENABLED(ENCODER_RATE_MULTIPLIER)
    #define MENU_MULTIPLIER_ITEM_EDIT(type, label, ...) MENU_MULTIPLIER_ITEM(setting_edit_ ## type, label, label, ## __VA_ARGS__)
    #define MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(type, label, ...) MENU_MULTIPLIER_ITEM(setting_edit_callback_ ## type, label, label, ## __VA_ARGS__)
  #else // !ENCODER_RATE_MULTIPLIER
    #define MENU_MULTIPLIER_ITEM_EDIT(type, label, ...) MENU_ITEM(setting_edit_ ## type, label, label, ## __VA_ARGS__)
    #define MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(type, label, ...) MENU_ITEM(setting_edit_callback_ ## type, label, label, ## __VA_ARGS__)
  #endif // !ENCODER_RATE_MULTIPLIER
...
void kill_screen(const char* lcd_msg) {
   ...
   */
    ...
    void _lcd_babystep(const AxisEnum axis, const char* msg) {
      ...
      void _lcd_babystep_x() { _lcd_babystep(X_AXIS, msg_babystep_x()); }
      void _lcd_babystep_y() { _lcd_babystep(Y_AXIS, msg_babystep_y()); }
      ...
      void lcd_babystep_zoffset() {
        ...
        if (lcdDrawUpdate) {
          lcd_implementation_drawedit(msg_zprobe_zoffset(), ftostr43sign(zprobe_zoffset));
          #if ENABLED(BABYSTEP_ZPROBE_GFX_OVERLAY)
      ...
      void _lcd_babystep_z() { _lcd_babystep(Z_AXIS, msg_babystep_z()); }
  ...
  void lcd_tune_menu() {
    ...
    #else // HOTENDS > 1
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, msg_nozzle1(), &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15, watch_temp_callback_E0);
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, msg_nozzle2(), &thermalManager.target_temperature[1], 0, HEATER_1_MAXTEMP - 15, watch_temp_callback_E1);
      #if HOTENDS > 2
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, msg_nozzle3(), &thermalManager.target_temperature[2], 0, HEATER_2_MAXTEMP - 15, watch_temp_callback_E2);
        #if HOTENDS > 3
          MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, msg_nozzle4(), &thermalManager.target_temperature[3], 0, HEATER_3_MAXTEMP - 15, watch_temp_callback_E3);
          #if HOTENDS > 4
            MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, msg_nozzle5(), &thermalManager.target_temperature[4], 0, HEATER_4_MAXTEMP - 15, watch_temp_callback_E4);
          #endif // HOTENDS > 4
        #endif // HOTENDS > 3
      #endif // HOTENDS > 2
    #endif // HOTENDS > 1
    ...
    //
      ...
      #endif
      #if HAS_FAN1
        MENU_MULTIPLIER_ITEM_EDIT(int3, msg_fan_speed2(), &fanSpeeds[1], 0, 255);
      #endif
      #if HAS_FAN2
        MENU_MULTIPLIER_ITEM_EDIT(int3, msg_fan_speed3(), &fanSpeeds[2], 0, 255);
      #endif
    #endif // FAN_COUNT > 0
    ...
    //
      ...
      MENU_ITEM_EDIT(int3, msg_flow1(), &flow_percentage[0], 10, 999);
      MENU_ITEM_EDIT(int3, msg_flow2(), &flow_percentage[1], 10, 999);
      #if EXTRUDERS > 2
        MENU_ITEM_EDIT(int3, msg_flow3(), &flow_percentage[2], 10, 999);
        #if EXTRUDERS > 3
          MENU_ITEM_EDIT(int3, msg_flow4(), &flow_percentage[3], 10, 999);
          #if EXTRUDERS > 4
            MENU_ITEM_EDIT(int3, msg_flow5(), &flow_percentage[4], 10, 999);
          #endif // EXTRUDERS > 4
        #endif // EXTRUDERS > 3
      #endif // EXTRUDERS > 2
    #endif // EXTRUDERS > 1
   ...
   */
    ...
    void lcd_dac_menu() {
      ...
      MENU_ITEM_EDIT_CALLBACK(int8, msg_drive_strengthx(), &driverPercent[X_AXIS], 0, 100, dac_driver_commit);
      MENU_ITEM_EDIT_CALLBACK(int8, msg_drive_strengthy(), &driverPercent[Y_AXIS], 0, 100, dac_driver_commit);
      MENU_ITEM_EDIT_CALLBACK(int8, msg_drive_strengthz(), &driverPercent[Z_AXIS], 0, 100, dac_driver_commit);
      MENU_ITEM_EDIT_CALLBACK(int8, msg_drive_strengthe(), &driverPercent[E_AXIS], 0, 100, dac_driver_commit);
  ...
  void _lcd_preheat(const int16_t endnum, const int16_t temph, const int16_t tempb, const int16_t fan) {
    ...
    void lcd_preheat_m1_menu() {
      ...
      #else
        #if TEMP_SENSOR_BED != 0
          MENU_ITEM(function, msg_preheat_1_n1(), lcd_preheat_m1_e0);
          MENU_ITEM(function, msg_preheat_1_end1(), lcd_preheat_m1_e0_only);
          MENU_ITEM(function, msg_preheat_1_n2(), lcd_preheat_m1_e1);
          MENU_ITEM(function, msg_preheat_1_end2(), lcd_preheat_m1_e1_only);
        #else
          MENU_ITEM(function, msg_preheat_1_n1(), lcd_preheat_m1_e0_only);
          MENU_ITEM(function, msg_preheat_1_n2(), lcd_preheat_m1_e1_only);
        #endif
        #if HOTENDS > 2
          #if TEMP_SENSOR_BED != 0
            MENU_ITEM(function, msg_preheat_1_n3(), lcd_preheat_m1_e2);
            MENU_ITEM(function, msg_preheat_1_end3(), lcd_preheat_m1_e2_only);
          #else
            MENU_ITEM(function, msg_preheat_1_n3(), lcd_preheat_m1_e2_only);
          #endif
          #if HOTENDS > 3
            #if TEMP_SENSOR_BED != 0
              MENU_ITEM(function, msg_preheat_1_n4(), lcd_preheat_m1_e3);
              MENU_ITEM(function, msg_preheat_1_end4(), lcd_preheat_m1_e3_only);
            #else
              MENU_ITEM(function, msg_preheat_1_n4(), lcd_preheat_m1_e3_only);
            #endif
            #if HOTENDS > 4
              #if TEMP_SENSOR_BED != 0
                MENU_ITEM(function, msg_preheat_1_n5(), lcd_preheat_m1_e4);
                MENU_ITEM(function, msg_preheat_1_end5(), lcd_preheat_m1_e4_only);
              #else
                MENU_ITEM(function, msg_preheat_1_n5(), lcd_preheat_m1_e4_only);
              #endif
            #endif // HOTENDS > 4
          #endif // HOTENDS > 3
        #endif // HOTENDS > 2
    ...
    void lcd_preheat_m2_menu() {
      ...
      #else
        #if TEMP_SENSOR_BED != 0
          MENU_ITEM(function, msg_preheat_2_n1(), lcd_preheat_m2_e0);
          MENU_ITEM(function, msg_preheat_2_end1(), lcd_preheat_m2_e0_only);
          MENU_ITEM(function, msg_preheat_2_n2(), lcd_preheat_m2_e1);
          MENU_ITEM(function, msg_preheat_2_end2(), lcd_preheat_m2_e1_only);
        #else
          MENU_ITEM(function, msg_preheat_2_n1(), lcd_preheat_m2_e0_only);
          MENU_ITEM(function, msg_preheat_2_n2(), lcd_preheat_m2_e1_only);
        #endif
        #if HOTENDS > 2
          #if TEMP_SENSOR_BED != 0
            MENU_ITEM(function, msg_preheat_2_n3(), lcd_preheat_m2_e2);
            MENU_ITEM(function, msg_preheat_2_end3(), lcd_preheat_m2_e2_only);
          #else
            MENU_ITEM(function, msg_preheat_2_n3(), lcd_preheat_m2_e2_only);
          #endif
          #if HOTENDS > 3
            #if TEMP_SENSOR_BED != 0
              MENU_ITEM(function, msg_preheat_2_n4(), lcd_preheat_m2_e3);
              MENU_ITEM(function, msg_preheat_2_end4(), lcd_preheat_m2_e3_only);
            #else
              MENU_ITEM(function, msg_preheat_2_n4(), lcd_preheat_m2_e3_only);
            #endif
            #if HOTENDS > 4
              #if TEMP_SENSOR_BED != 0
                MENU_ITEM(function, msg_preheat_2_n5(), lcd_preheat_m2_e4);
                MENU_ITEM(function, msg_preheat_2_end5(), lcd_preheat_m2_e4_only);
              #else
                MENU_ITEM(function, msg_preheat_2_n5(), lcd_preheat_m2_e4_only);
              #endif
            #endif // HOTENDS > 4
          #endif // HOTENDS > 3
        #endif // HOTENDS > 2
  ...
  void lcd_cooldown() {
    ...
    void _lcd_after_probing() {
      ...
      // Display "Done" screen and wait for moves to complete
        ...
        lcd_synchronize(msg_level_bed_done());
      #endif
      lcd_goto_previous_menu();
      lcd_completion_feedback();
      defer_return_to_status = false;
    ...
    void _lcd_level_bed_homing() {
      if (lcdDrawUpdate) lcd_implementation_drawedit(msg_level_bed_homing(), NULL);
      lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
    ...
    void _lcd_ubl_map_homing() {
      ...
      if (lcdDrawUpdate) lcd_implementation_drawmenu_static(LCD_HEIGHT < 3 ? 0 : (LCD_HEIGHT > 4 ? 2 : 1), msg_level_bed_homing());
      lcdDrawUpdate = LCDVIEW_CALL_NO_REDRAW;
      if (axis_homed[X_AXIS] && axis_homed[Y_AXIS] && axis_homed[Z_AXIS])
...
static void lcd_autohome()
{
  ...
  float move_menu_scale;
    ...
    void _lcd_calibrate_homing() {
      if (lcdDrawUpdate) lcd_implementation_drawmenu_static(LCD_HEIGHT >= 4 ? 1 : 0, msg_level_bed_homing());
      lcdDrawUpdate = LCDVIEW_CALL_REDRAW_NEXT;
      if (axis_homed[X_AXIS] && axis_homed[Y_AXIS] && axis_homed[Z_AXIS])
        lcd_goto_previous_menu();
    }
  ...
  void _lcd_move_e(
    ...
    if (lcdDrawUpdate && !processing_manual_move) {
      ...
      #else
        switch (eindex) {
          default: pos_label = msg_move_e1(); break;
          case 1: pos_label = msg_move_e2(); break;
          #if E_MANUAL > 2
            case 2: pos_label = msg_move_e3(); break;
            #if E_MANUAL > 3
              case 3: pos_label = msg_move_e4(); break;
              #if E_MANUAL > 4
                case 4: pos_label = msg_move_e5(); break;
              #endif // E_MANUAL > 4
            #endif // E_MANUAL > 3
          #endif // E_MANUAL > 2
        }
  ...
  void lcd_move_menu()
  {
    ...
    if (_MOVE_XYZ_ALLOWED)
    {
      ...
      if (active_extruder)
        MENU_ITEM(gcode, msg_select1(), PSTR("T0"));
      else
        MENU_ITEM(gcode, msg_select2(), PSTR("T1"));
    #endif

    MENU_ITEM(submenu, msg_move_e(), lcd_move_get_e_amount);
    #if E_MANUAL > 1
      MENU_ITEM(submenu, msg_move_e1(), lcd_move_get_e0_amount);
      MENU_ITEM(submenu, msg_move_e2(), lcd_move_get_e1_amount);
      #if E_MANUAL > 2
        MENU_ITEM(submenu, msg_move_e3(), lcd_move_get_e2_amount);
        #if E_MANUAL > 3
          MENU_ITEM(submenu, msg_move_e4(), lcd_move_get_e3_amount);
          #if E_MANUAL > 4
            MENU_ITEM(submenu, msg_move_e5(), lcd_move_get_e4_amount);
          #endif // E_MANUAL > 4
        #endif // E_MANUAL > 3
      #endif // E_MANUAL > 2
    #endif // E_MANUAL > 1
...
*/
    ...
    //
    MENU_ITEM(submenu, msg_preheat_1_setting(), lcd_control_temperature_preheat_material1_settings_menu);
  ...
  void lcd_control_motion_velocity_menu() {
    ...
    // M203 Max Feedrate
    MENU_ITEM_EDIT(float3, msg_vmaxx(), &planner.max_feedrate_mm_s[X_AXIS], 1, 999);
    MENU_ITEM_EDIT(float3, msg_vmaxy(), &planner.max_feedrate_mm_s[Y_AXIS], 1, 999);
    MENU_ITEM_EDIT(float3, msg_vmaxz(), &planner.max_feedrate_mm_s[Z_AXIS], 1, 999);

    #if ENABLED(DISTINCT_E_FACTORS)
      MENU_ITEM_EDIT(float3, msg_vmaxe(), &planner.max_feedrate_mm_s[E_AXIS + active_extruder], 1, 999);
      MENU_ITEM_EDIT(float3, msg_vmaxe1(), &planner.max_feedrate_mm_s[E_AXIS], 1, 999);
      MENU_ITEM_EDIT(float3, msg_vmaxe2(), &planner.max_feedrate_mm_s[E_AXIS + 1], 1, 999);
      #if E_STEPPERS > 2
        MENU_ITEM_EDIT(float3, msg_vmaxe3(), &planner.max_feedrate_mm_s[E_AXIS + 2], 1, 999);
        #if E_STEPPERS > 3
          MENU_ITEM_EDIT(float3, msg_vmaxe4(), &planner.max_feedrate_mm_s[E_AXIS + 3], 1, 999);
          #if E_STEPPERS > 4
            MENU_ITEM_EDIT(float3, msg_vmaxe5(), &planner.max_feedrate_mm_s[E_AXIS + 4], 1, 999);
          #endif // E_STEPPERS > 4
        #endif // E_STEPPERS > 3
      #endif // E_STEPPERS > 2
    #else
      MENU_ITEM_EDIT(float3, msg_vmaxe(), &planner.max_feedrate_mm_s[E_AXIS], 1, 999);
    #endif
  ...
  void lcd_control_motion_acceleration_menu() {
    ...
    // M201 settings
    MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxx(), &planner.max_acceleration_mm_per_s2[X_AXIS], 100, 99000, _reset_acceleration_rates);
    MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxy(), &planner.max_acceleration_mm_per_s2[Y_AXIS], 100, 99000, _reset_acceleration_rates);
    MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxz(), &planner.max_acceleration_mm_per_s2[Z_AXIS], 10, 99000, _reset_acceleration_rates);

    #if ENABLED(DISTINCT_E_FACTORS)
      MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxe(), &planner.max_acceleration_mm_per_s2[E_AXIS + active_extruder], 100, 99000, _reset_acceleration_rates);
      MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxe1(), &planner.max_acceleration_mm_per_s2[E_AXIS], 100, 99000, _reset_e0_acceleration_rate);
      MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxe2(), &planner.max_acceleration_mm_per_s2[E_AXIS + 1], 100, 99000, _reset_e1_acceleration_rate);
      #if E_STEPPERS > 2
        MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxe3(), &planner.max_acceleration_mm_per_s2[E_AXIS + 2], 100, 99000, _reset_e2_acceleration_rate);
        #if E_STEPPERS > 3
          MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxe4(), &planner.max_acceleration_mm_per_s2[E_AXIS + 3], 100, 99000, _reset_e3_acceleration_rate);
          #if E_STEPPERS > 4
            MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxe5(), &planner.max_acceleration_mm_per_s2[E_AXIS + 4], 100, 99000, _reset_e4_acceleration_rate);
          #endif // E_STEPPERS > 4
        #endif // E_STEPPERS > 3
      #endif // E_STEPPERS > 2
    #else
      MENU_ITEM_EDIT_CALLBACK(long5, msg_amaxe(), &planner.max_acceleration_mm_per_s2[E_AXIS], 100, 99000, _reset_acceleration_rates);
    #endif
  ...
  void lcd_control_filament_menu() {
    ...
    if (volumetric_enabled) {
      ...
      #else // EXTRUDERS > 1
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, msg_filament_diam1(), &filament_size[0], 1.5, 3.25, calculate_volumetric_multipliers);
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, msg_filament_diam2(), &filament_size[1], 1.5, 3.25, calculate_volumetric_multipliers);
        #if EXTRUDERS > 2
          MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, msg_filament_diam3(), &filament_size[2], 1.5, 3.25, calculate_volumetric_multipliers);
          #if EXTRUDERS > 3
            MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, msg_filament_diam4(), &filament_size[3], 1.5, 3.25, calculate_volumetric_multipliers);
            #if EXTRUDERS > 4
              MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, msg_filament_diam5(), &filament_size[4], 1.5, 3.25, calculate_volumetric_multipliers);
            #endif // EXTRUDERS > 4
          #endif // EXTRUDERS > 3
        #endif // EXTRUDERS > 2
      #endif // EXTRUDERS > 1
   ...
   */
    ...
    void lcd_sdcard_menu() {
      ...
      void lcd_info_stats_menu() {
        ...
        STATIC_ITEM(PSTR(""), false, false, buffer);                                                         // 99y 364d 23h 59m 59s
        ...
        STATIC_ITEM(PSTR(""), false, false, buffer);                                                         // 125m
        END_SCREEN();
      }
    #endif // PRINTCOUNTER
    ...
    void lcd_info_printer_menu() {
      ...
      STATIC_ITEM(PSTR(MSG_DETAILED_BUILD_VERSION), true);             // Creality 3D
      STATIC_ITEM(PSTR(MSG_SHORT_BUILD_VERSION), true);                // x.x.x-Branch
      // STATIC_ITEM(STRING_DISTRIBUTION_DATE, true);                  // YYYY-MM-DD HH:MM
      STATIC_ITEM(PSTR(MSG_MACHINE_NAME), true);                       // CR-10S
      STATIC_ITEM(PSTR(MSG_WEBSITE_URL), true);                        // www.creality3d.cn
      STATIC_ITEM(msg_info_extruders(), true);                         // Extruders: 1
      STATIC_ITEM(PSTR(MSG_BOARD_NAME), true);                         // MyPrinterController
      STATIC_ITEM(msg_info_baudrate(), true);                          // Baud: 115200
      /*
      #if ENABLED(AUTO_BED_LEVELING_3POINT)
   ...
   */
    ...
    #define HOTEND_STATUS_ITEM() do { \
      if (_menuLineNr == _thisItemNr) { \
        if (lcdDrawUpdate) { \
          lcd_implementation_drawmenu_static(_lcdLineNr, msg_filament_change_nozzle(), false, true); \
    ...
    void lcd_advanced_pause_toocold_menu() {
      ...
      STATIC_ITEM(msg_filament_change_mintemp(), false, false);
      MENU_BACK(msg_back());
      #if LCD_HEIGHT > 4
        STATIC_ITEM(" ");
      #endif
...
void lcd_update()
{
  ...
  */
  ...
  {
    if((0 == READ(CHECK_MATWEIAL)) && (false == sdcard_pause_check))
    {
      checkpause ++;
      if(checkpause > 10)
      {
    ...
    else if((1 == READ(CHECK_MATWEIAL)) && (true == sdcard_pause_check))
    {
      checknum ++;
      if(checknum > 10)
      {
    ...
    if (sd_status != lcd_sd_status && lcd_detected())
    {
      if (sd_status)
      {
        ...
        if (lcd_sd_status != 2)
        {
          lcd_synchronize(msg_sd_inserted());
        }
        #if ENABLED(SDSUPPORT) && ENABLED(POWEROFF_SAVE_SD_FILE)
          init_power_off_info();
        #endif
      ...
      else
      {
        ...
        if (lcd_sd_status != 2)
        {
          lcd_synchronize(msg_sd_removed());
        }
      }
